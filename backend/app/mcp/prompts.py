...
def swap_product_safely(app_id: int, product_id: str, new_product_id: str) -> str:
    ...
    return (
        ...
        "1. Call `pricing_list_subscriptions` (or `pricing_list_iaps`) to find "
        f"the local id for product '{product_id}'.\n"
        "2. Call the matching `swap_subscription_product` or `swap_iap` tool "
...
def optimize_keywords(app_id: int, locale: str = "en-US") -> str:
    ...
    return (
        ...
        "Steps:\n\n"
        "1. Call `aso_aso_check` to see current metadata gaps.\n"
        "2. Call `metadata_get_snapshot` to read existing title/subtitle/keywords/description.\n"
        "3. Call `keywords_list_for_app` to see currently tracked keywords + rankings.\n"
        "4. Call `keywords_search` and `keywords_suggestions` to find new candidates.\n"
        "5. Call `clash_run` to compare against competitor keyword usage.\n"
        "6. Propose concrete edits as a list of `{field, current, proposed, rationale}` "
        "rows. Do NOT call `metadata_update_locale` until the user approves.\n"
    )
