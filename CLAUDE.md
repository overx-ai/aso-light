...
- **Error messages**: never expose raw Python errors to API responses; use `HTTPException`. Signal local guard failures with a dedicated exception type (`ASCRequestInvalidError`, `IAPScheduleUnsyncedError`, `ChildResourceNotFoundError` in `app/services/asc/errors.py`) — **never a bare `ValueError`**: `json.JSONDecodeError` is a `ValueError`, so a caller catching it around any JSON-parsing service call turns a malformed upstream body into a confident 4xx whose detail is a raw Python message
...
- **DB init**: migration-first in all environments — app startup runs Alembic `upgrade head` before seeding; `make db-up && make migrate` is the explicit preflight path
...
- **MCP server**: mounted at `/mcp` on the FastAPI app, exposes 187 tools across all REST domains via `fastmcp`, each carrying a `readOnlyHint`/`destructiveHint` annotation from the allowlists in `backend/app/mcp/consent.py` (unclassified = keeps prompting). Auth uses Personal Access Tokens (`aso_pat_…`, sha256-hashed at rest, model in `backend/app/models/personal_access_token.py`, routes under `/api/v1/auth/tokens`). Tool modules live in `backend/app/mcp/tools/` and call service classes directly (no HTTP hop). App-scope is enforced by `app.mcp.context.resolve_app` mirroring `_get_verified_app`.
...
- **IAP immutables**: `productId` and `inAppPurchaseType` are immutable in ASC after create — `IAPUpdate` cannot express them. IAP mutations are **v2-only** (`/v2/inAppPurchases/{id}`) but their localizations are still v1 (`/v1/inAppPurchaseLocalizations/{id}`). `family_sharable` is honoured only on `NON_CONSUMABLE`; both create (`IAPCreate`) and update (`_assert_family_sharing_supported`, which reads the immutable locally-mirrored `iap_type`) reject it elsewhere rather than silently dropping it. Apple spells the attribute **`familySharable`** — one `e`, not `familyShareable`; the misspelling is rejected outright as a field name on reads and silently ignored on writes.
- **A never-priced IAP has no `iapPriceSchedule`**: Apple 404s the schedule GET, with a body byte-identical to the one for a nonexistent IAP — do not try to tell them apart from the error text. `get_iap_price_schedule` probes `get_iap_detail` and returns `[]` if the IAP answers. Apply refuses an empty local cache only when Apple actually holds prices (`_assert_iap_schedule_replaceable`), so a freshly created IAP needs no sync at all: create → preview → apply.
...
- [docs/017-iap-lifecycle-and-price-schedules.md](docs/017-iap-lifecycle-and-price-schedules.md) — IAP write paths, `familySharable`, the never-priced 404 probe, the whole-schedule-replace apply guard
